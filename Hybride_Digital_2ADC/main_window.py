...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hui & Rongrong & Gemini 的ADC监控上位机 v2.0")
        self.setGeometry(100, 100, 600, 400) 
        if not os.path.exists(LOG_DIR):
            os.makedirs(LOG_DIR)
        self.processor = DataProcessor()
        # self.processor.data_updated.connect(self.update_voltage_displays)
        self.processor.data_updated.connect(self.update_displays) # [修改] 连接到新的槽函数
        ...
        self.setup_display_area() # [修改] 调用新的UI创建函数
    ...
    # --- [核心修改] 创建新的、包含电压和物理量的显示区域 ---
    def setup_display_area(self):
        """创建包含电压和物理量的显示区域"""
        grid_layout = QGridLayout()
        grid_layout.setColumnStretch(1, 1) # 让第二列（数值）可以伸展
        grid_layout.setColumnStretch(3, 1) # 让第四列（数值）可以伸展

        # --- O2 (Temperature) - CH1 ---
        # 标签
        o2_label = QLabel("O2 Sensor (CH1)")
        o2_label.setFont(QFont("Arial", 12, QFont.Bold))
        grid_layout.addWidget(o2_label, 0, 0, 1, 4) # 跨4列

        # 电压显示
        grid_layout.addWidget(QLabel("  Voltage:"), 1, 0)
        self.display_o2_voltage = QLineEdit("-.---")
        self.display_o2_voltage.setReadOnly(True)
        self.display_o2_voltage.setFont(QFont("Consolas", 12))
        grid_layout.addWidget(self.display_o2_voltage, 1, 1)
        grid_layout.addWidget(QLabel("V"), 1, 2) # 独立的单位标签

        # 温度显示
        grid_layout.addWidget(QLabel("  Temperature:"), 2, 0)
        self.display_o2_temp = QLineEdit("---.-")
        self.display_o2_temp.setReadOnly(True)
        self.display_o2_temp.setFont(QFont("Consolas", 12))
        grid_layout.addWidget(self.display_o2_temp, 2, 1)
        grid_layout.addWidget(QLabel("℃"), 2, 2) # 独立的单位标签

        # --- 分隔线 ---
        line = QFrame()
        line.setFrameShape(QFrame.HLine)
        line.setFrameShadow(QFrame.Sunken)
        grid_layout.addWidget(line, 3, 0, 1, 4)

        # --- O1 (Pressure) - CH2 ---
        # 标签
        o1_label = QLabel("O1 Sensor (CH2)")
        o1_label.setFont(QFont("Arial", 12, QFont.Bold))
        grid_layout.addWidget(o1_label, 4, 0, 1, 4)

        # 电压显示
        grid_layout.addWidget(QLabel("  Voltage:"), 5, 0)
        self.display_o1_voltage = QLineEdit("-.---")
        self.display_o1_voltage.setReadOnly(True)
        self.display_o1_voltage.setFont(QFont("Consolas", 12))
        ...
        self.display_o1_pressure.setReadOnly(True)
        self.display_o1_pressure.setFont(QFont("Consolas", 12))
    ...
    #    """创建8个通道的电压显示网格，并禁用CH4/5"""
    #    grid_layout = QGridLayout()
    #    self.voltage_displays = []
        
    #    for i in range(8):
    #        ch_num = i + 1
    #        channel_label = QLabel(f"CH{ch_num} Voltage:")
    ...
    #            voltage_display.setText("N/A")
    #            voltage_display.setEnabled(False)
